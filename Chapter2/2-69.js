...
function successive_merge(pairs){
    return is_null(tail(pairs))
        ? head(pairs)
        : successive_merge(adjoin_set(make_code_tree(head(pairs), head(tail(pairs))), tail(tail(pairs))));
}
...
display_list(generate_huffman_tree(sample_frequencies));
equal(sample_tree, 
